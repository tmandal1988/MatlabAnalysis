...
% Last Modified by GUIDE v2.5 06-Jul-2015 23:59:19
...
handles.SendBuf=zeros(10,1);
handles.SendBuf(1)=170;handles.SendBuf(2)=171;handles.SendBuf(3)=187;
handles.serialObject=0;
...
%      'TimerFcn', {@send_data,hObject}); % Specify callback
...
if(isnan(handles.delayval) || handles.delayval<0 || handles.delayval>800)
...
handles.SendBuf=[170 171 187 0 0 0 0 0 0 0];

dataBuf=typecast(int16(handles.delayval*10),'uint8');
handles.SendBuf(4)=dataBuf(1);
handles.SendBuf(5)=dataBuf(2);
dataBuf=typecast(int16(handles.rateval*100),'uint8');
handles.SendBuf(6)=dataBuf(1);
handles.SendBuf(7)=dataBuf(2);
dataBuf=typecast(int16(handles.trimval*100),'uint8');
handles.SendBuf(8)=dataBuf(1);
handles.SendBuf(9)=dataBuf(2);


ByteSum=typecast(uint16(handles.SendBuf(4)+handles.SendBuf(5)+handles.SendBuf(6)+handles.SendBuf(7)+handles.SendBuf(8)+handles.SendBuf(9)),'uint8');
handles.SendBuf(10)=bitxor(ByteSum(1),255);

handles.GCSdata(1)=handles.SendBuf(4);
handles.GCSdata(2)=handles.SendBuf(5);
handles.GCSdata(3)=handles.SendBuf(6);
handles.GCSdata(4)=handles.SendBuf(7);
handles.GCSdata(5)=handles.SendBuf(8);
handles.GCSdata(6)=handles.SendBuf(9);

for i=1:5
    fwrite(handles.serialObject,handles.SendBuf,'uint8');
    pause(0.01);
end
...
if (strcmp(get(hObject,'String'),'Open'))
    if(strcmp(handles.serialportname,'Select a Serial Port'))
        ...
        handles.serialObject.InputBufferSize=1024;
        handles.serialObject.BytesAvailableFcnCount = 50;
        handles.serialObject.BytesAvailableFcnMode = 'byte';
        handles.serialObject.BytesAvailableFcn = {@update_display,hObject};
%         handles.serialObject.InputBufferSize=4096;
%         handles.serialObject.ReadAsyncMode='continuous';
...
if(isnan(handles.trimval) || handles.trimval<-10 || handles.trimval>10)
    display('Please for the love of good enter a valid value between -10 and 10')
    handles.trimval=0;
    set(hObject,'String','0')
end
...
ReadAndModifyDisplay(handles.serialObject,handles);

% function send_data(hObject,eventdata,hfigure)
% % Timer timer1 callback, called each time timer iterates.
% % Gets surface Z data, adds noise, and writes it back to surface object.
% handles = guidata(hfigure);
% if(handles.serialObject~=0)
% if(strcmp(handles.serialObject.status,'open'))
%     fwrite(handles.serialObject,handles.SendBuf,'uint8');
% end
% end
...
% --- Executes during object creation, after setting all properties.
function LoopBackDisplay_CreateFcn(hObject, eventdata, handles)
% hObject    handle to LoopBackDisplay (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
