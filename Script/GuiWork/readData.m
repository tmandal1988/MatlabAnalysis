function readData(serialObject,event)
...
if(n~=0)
    ...
    count,readPointer,writePointer
%     for i=1:count
%         writePointer=mod((writePointer+1),1000);
%         RawDataBuf(writePointer)=RecBuf(i);
%     end
...
if(writePointer-readPointer >= 138)
    
    
    uint8(RawDataBuf(readPointer)),uint8(RawDataBuf(mod((readPointer+1),1000))),uint8(RawDataBuf(mod((readPointer+2),1000)))
    if(uint8(RawDataBuf(readPointer))==170 && uint8(RawDataBuf(mod((readPointer+1),1000)))==171 && uint8(RawDataBuf(mod((readPointer+2),1000)))==187)
...
%        GCSdata(1)=RawDataBuf(mod((readPointer+107),1000));
%        GCSdata(2)=RawDataBuf(mod((readPointer+108),1000));
%        GCSdata(3)=RawDataBuf(mod((readPointer+109),1000));
%        GCSdata(4)=RawDataBuf(mod((readPointer+110),1000));
%        GCSdata(5)=RawDataBuf(mod((readPointer+111),1000));
%        GCSdata(6)=RawDataBuf(mod((readPointer+112),1000));
%        
%       %handles.GCSdata(2)
...
% %         display('-------------------------')
%        
%        if(handles.GCSdata(1)==GCSdata(1) && handles.GCSdata(2)==GCSdata(2) && handles.GCSdata(3)==GCSdata(3) && handles.GCSdata(4)==GCSdata(4) && handles.GCSdata(5)==GCSdata(5) && handles.GCSdata(6)==GCSdata(6))
%       
%          display('hahaha')  
%        end
...
%        
        readPointer=mod((readPointer+139),1000);
     else
       readPointer=mod((readPointer+1),1000);
    end
%      
% end
%       
